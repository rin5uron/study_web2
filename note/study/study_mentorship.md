 # 【2025/12/08】
 - ブラウザ操作の可能性
 　- ブラウザ操作することでその情報を引き抜ける
 　
 # 【2025/12/07】
 - claude
 - 公式の出しているプラグインとスキル図を見る
 


 - サイトマップ、Googleにコンテンツの認識をしてもらう
 SEO対策クローラー対策
 https://slo-map.com/sitemap.xml
 - このサイトのプライバシーポリシーロボットに対して
 https://slo-map.com/robots.txt
 - スクレイピングは悪意があるかないか
 - この事件以後特に話題になった事件は現れていない
 岡崎市立中央図書館事件（Librahack事件）の概要

  事件の経緯：
  - 2010年3月：愛知県在住の男性が、岡崎市立中央図書館の新着図書検索システム（OPAC）から新刊情報を自動取得するプログラムを実行
  - 2010年5月25日：この男性が「業務妨害罪（偽計業務妨害）」の容疑で逮捕される
  - 容疑内容は「図書館のウェブサイトに大量アクセスを繰り返し、システムの一部を利用不能にした」というもの

  事件の背景：
  - 男性は約1秒に1回程度のアクセス頻度でプログラムを実行していた
  - 図書館側のシステムに脆弱性があり、通常のアクセスでも負荷に耐えられない状態だった
  - 男性に攻撃の意図はなく、新着図書情報を効率的に収集する目的だった

  最終的な結果：
  無罪判決ではなく、不起訴処分（起訴猶予処分）となりました

  - 検察が起訴しないことを決定したため、裁判は行われず、したがって無罪判決も出ていません
  - 不起訴処分 = 検察が訴追しないと判断
  - 無罪判決 = 裁判で無罪が確定

  影響：
  この事件は、Webスクレイピングの法的リスクやシステム側の責任について大きな議論を巻き起こしました。本来はシステムの脆弱性が問題だったにも関わらず、利用者が逮捕されたことで、エ
  ンジニアコミュニティに大きな不安を与えた事件として知られています。

  Sources:
  - https://ja.wikipedia.org/wiki/岡崎市立中央図書館事件
  - http://librahack.jp/
  - https://security.srad.jp/story/10/06/21/118216/
  - https://hatenanews.com/articles/201009/1696
...
- スプレッドシートとclaudeを
サービスアカウント
mcp使ってスラックとドライブにcclaude繋げられる
  - できること：データの読み取り・分析、書き込み・更新、自動化タスク、データ変換、プロジェクト管理
- git reset
  - タイムスリップに例えると
    - **--soft**：記憶を持ったままタイムスリップ（変更内容はステージングに残る）
    - **--mixed**（デフォルト）：記憶はあるけど整理されていない（変更は作業ディレクトリに残る）
    - **--hard**：記憶喪失でタイムスリップ（変更内容が完全に消える）⚠️危険
  - 使い分け
    - soft：コミットメッセージ書き直し、複数コミットをまとめたい
    - mixed：変更を残しつつコミット履歴だけ戻したい
    - hard：間違った変更を完全に捨てたい（注意！復元困難）
# 【2025/10/08】
- SESはやめとけ
　- キャリアをガチャで回すな
　- せめて受託開発にしろ
...
- なんでジャバスクリプトなのか
　- 作りたいものを作ろうと思ったらJavaScriptだったから
　- やっていく中で、難しい面もあると感じ、本格的なサイトを作るにはフレームワークも必要だと感じ、
色々調べた結果、ネクストjsを今実務や学習で使っている
