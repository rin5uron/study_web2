...
**特徴:**
- ユーザーインタラクションに対する視覚的フィードバックを提供
- デフォルトでは色の変化は即時（アニメーションなし）
- 滑らかな変化を付けるにはtransitionプロパティと組み合わせる

**transitionプロパティとの組み合わせ:**
```css
.button {
...
上記の例では、ボタンにマウスを乗せると0.3秒かけて背景色と文字色が徐々に変化します。

**cursor: pointerプロパティ**

`cursor`プロパティはマウスカーソルが要素上にあるときの形状を指定します。`pointer`値を設定すると、手のひら型のポインターに変化し、要素がクリック可能であることを示します。

**使用例:**
```css
.button {
  cursor: pointer; /* クリック可能であることを示す手のひらカーソルを表示 */
}
```
...
**ポイント:**
- シンプルながら重要なUX（ユーザーエクスペリエンス）要素
- クリック可能な要素には基本的に設定する
- 視覚的なフィードバックとナビゲーション補助の役割を果たす

**他のcursor値:**
- `default` - 標準の矢印
- `text` - テキスト選択用のI型
- `wait` - 読み込み中を示す砂時計や円
- `not-allowed` - 禁止を示す円に斜線
- `move` - 移動可能を示す十字矢印
