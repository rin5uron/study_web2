...
## CSSプロパティの継承に関する注意点 📝
### transform-originの適用エラー（2025/04/20）
- 問題の現象
  - 親のdiv要素に`transform-origin: top left`を設定しても、子のimg要素に効果が反映されなかった
- 原因
  - `transform-origin`は継承されないCSSプロパティの1つ
  - 変形の基準点は、変形（transform）を適用する要素自体に直接指定する必要がある
- 解決方法
  - 変形を適用したい要素（この場合はimg要素）に直接`transform-origin`を設定
  ```html
  <img src="..." style="transform-origin: top left;">
  ```
- 学んだこと
  - CSSプロパティには2種類ある：
    1. 親から子に継承される（例：`color`、`font-size`）
    2. 継承されない（例：`transform`、`transform-origin`）
  - 変形関連のプロパティは基本的に継承されない
  - エラーの際は、プロパティが継承されるタイプかどうかを確認する
...
## CSSセレクタの種類と使い分け 📝 2025/04/24

### IDセレクタとクラスセレクタの比較

| 特徴 | IDセレクタ (#) | クラスセレクタ (.) |
|------|--------------|-----------------|
| 記述方法 | `#id名 { プロパティ: 値; }` | `.クラス名 { プロパティ: 値; }` |
| HTML記述 | `id="id名"` | `class="クラス名"` |
| 一意性 | ページ内で同じIDは1つだけ使用可能 | 複数の要素に同じクラスを適用可能 |
| 詳細度 | 高い（上書きしにくい） | IDより低い（上書きしやすい） |
| 主な用途 | JavaScript用の要素特定、ページ内リンクの目印 | デザインや装飾のグループ化 |

### IDセレクタの特徴
- シャープ記号（`#`）で始まる
- 例：`#minus { font-size: 26px; }`
- 同じページ内で同じIDは1回しか使えない（一意であるべき）
- CSSの詳細度（優先順位）が高い
- ページ内リンク（アンカーリンク）の飛び先としても使われる
- JavaScriptで要素を取得する際によく使用される

### クラスセレクタの特徴
- ドット（`.`）で始まる
- 例：`.button { background-color: blue; }`
- 複数の要素に同じクラスを適用できる
- 1つの要素に複数のクラスを指定できる（例：`class="btn primary large"`)
- CSSの詳細度はIDより低い（クラス < ID）
- デザインの共通化やコンポーネント化に適している

### 実際の使い分け
- **IDの使用場面**:
  - ページ内で唯一の要素（ヘッダー、フッター、メインナビゲーションなど）
  - JavaScriptで操作する要素（カウンター、フォーム要素など）
  - ページ内リンクの目的地（`<a href="#section1">セクション1へ</a>`）

- **クラスの使用場面**:
  - 繰り返し使用するスタイル（ボタン、カード、見出しのスタイルなど）
  - 状態による見た目の変化（`.active`, `.disabled`など）
  - レイアウトの構造化（`.container`, `.flex-row`など）

### ベストプラクティス
1. IDはJavaScript用、クラスはCSS用と役割を分ける
2. IDは必要最小限に抑え、多くのスタイリングはクラスで行う
3. 同じスタイルを複数の要素に適用する場合は必ずクラスを使用する
4. セマンティックな名前付けを心がける（機能や目的を表す名前）

# 🎥 Macで画面収録した時の画質設定＆Finder再起動手順メモ

## ✅ 画質が悪かった原因
- Mac標準の「画面収録」（QuickTime Playerなど）は**デフォルトでは画質がやや低め**に保存されることがある
- 特に動きがある画面や細かい文字は、標準設定だと荒くなりやすい

## ✅ 画質改善のためにやったこと
- 収録時の画面サイズを適切に選ぶ（できるだけ**フルHD（1920x1080）以上**）
- 必要に応じて「高解像度で書き出し」（QuickTimeで[書き出し]→[1080p]などを選ぶ）
- macOSの「設定」で**録画品質を変更**する（アプリによっては詳細設定が必要）

## ✅ Finder表示が英語になっていた問題
- システム環境設定 →「言語と地域」で日本語が優先になっていても、Finderが古い情報を参照していることがあった

## ✅ Finderの再起動手順
1. `command` + `option` + `esc` を同時押し（「アプリ強制終了」ウインドウを開く）
2. 「Finder」を選択
3. 「Relaunch（再起動）」ボタンを押す
4. Finderが一度閉じて、すぐに再起動
5. 表示が日本語に正しく戻るか確認！

※ ファイル自体が消えたり壊れたりすることはないので安心してOK！

---

# 📝 学びポイントまとめ
- **デモ動画収録**は「録画時のサイズ」と「保存時の書き出し品質」が大事
- **システム設定を変えた後**は、必要に応じて「アプリの再起動」も忘れずに
- 作業メモをこまめに取ることで次回以降スムーズに進められる✨
（2025/4/29）


### JavaScriptの `eval()` とは？

- 文字列として書かれたJavaScriptコードを実行する関数
- 電卓アプリでは、入力された式（例："7+3*2"）を計算するのに使える
- 簡単に式を評価できるので、四則演算にはとても便利！
