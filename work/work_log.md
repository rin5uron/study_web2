...
## 【**2025/04/06**】HTML/CSSファイルの理解を深める
...
- スタイルの継承関係を把握することでより効率的なCSSが書ける
- レスポンシブデザインについてさらに学ぶ必要がある
...
## 【2025/04/05】CSS基礎課題：プロフィールカード改善

### 作業内容
- プロフィールカードのデザイン調整
- LEVELタグとLICENSEタグの位置調整
- 下部の枠の背景色を肌色に近い色に変更
- リンク先の設定とtarget属性の追加
- 矢印マークのデザイン改善

### 学んだこと
- `target="_blank"` 属性：リンクを新しいタブで開くための設定
  - 普通のリンク：クリックすると今見ているページが切り替わる
  - target="_blank"付きのリンク：クリックすると今のページはそのままで、新しいタブが開く

- `span`タグ：インライン要素の一種
  - テキストの一部だけを装飾したり操作したりするために使う
  - それ自体には意味がなく、CSSで装飾したりJavaScriptで制御するための「容器」として機能
  - 例：`<span class="portfolio-button">こちら</span>`

- `a`タグ：ハイパーリンクを作成するためのタグ
  - href属性でリンク先を指定
  - クリックすると別のページや別の場所に移動する機能を提供
  - 例：`<a href="https://github.com/rin5uron/study_web">リンク</a>`

### 困ったこと・解決策
- LEVELタグとLICENSEタグがうまく表示されなかった問題
  - border-leftの幅を調整し、適切なスペースを確保
  - 文字サイズとポジショニングを微調整して視認性を向上

- 矢印マークのデザイン改善
  - CSSの疑似要素（::before）を使って矢印を実装
  - 矢印の色を青系統に統一して全体のデザインとの調和を実現
...
| 用語 | 説明 | 例 |
|------|------|-----|
| target属性 | リンクを新しいタブやウィンドウで開くための属性 | `<a href="https://example.com" target="_blank">リンク</a>` |
| span要素 | インライン要素で、テキストの一部を囲んでスタイリングするための容器 | `<span class="highlight">強調テキスト</span>` |
| a要素 | アンカー要素。ハイパーリンクを作成するための要素 | `<a href="https://example.com">サイトへ移動</a>` |
| ハイパーリンク | クリックすると別のページや場所に移動できるウェブの基本機能 | テキストリンク、ボタン、画像リンクなど |
