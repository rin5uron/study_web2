...
### メモ 📌
- 学習時間：00:00-00:30（0時間30分）
- 学習内容：Node.jsとdate-fnsライブラリを使った日付処理の実装
- 合わせて学習時間：21:45-00:30（2時間45分）
...
## 2025/04/12（土）アコーディオン課題基礎編の続き（作業中）
...
### 完了したこと ✅
- JavaScript練習問題２の復習をチャット機能を使って実施
- JavaScript練習問題３のファイル作成、学習に取り掛かり

### 学んだこと 📝
- プログラミングでは配列のインデックスは0から始まる
- switch文でbreakがない場合、「フォールスルー」という現象が起こる。条件に一致したcase以降の全ての処理が実行されてしまう
- 二重ループでは外側のループが大きな単位、内側のループが小さな単位を担当（九九の例など）

### 気づき 💡
- この問題で復習問題をAIに使ってチャットで答えることで、実際のコードの理解、自分でコードを繰り返し書いて身につけることに繋がる
...
## 【2025/04/16】JavaScript超基礎の練習問題3

### 完了したこと ✅
- JavaScript超基礎の練習問題3を完了
- 関数定義と呼び出しの構文エラーを修正
- 文字列操作の誤りを修正（全角/半角の区別、テンプレートリテラルの使用）

### 学んだこと 📝
- 関数名と呼び出し名は完全に一致させる必要がある
- 日本語環境での全角/半角の区別が重要（特に括弧や演算子）
- 文字列への変数の挿入方法：
  - `+` 演算子による連結
  - テンプレートリテラル（バッククォート）による挿入
- 条件分岐の比較演算子の正しい使い方
- デバッグ時はコンソールのエラーメッセージを確認する習慣をつける

### 気づき・発見 💡
- 日本語入力時の全角/半角切り替えがエラーの主な原因になっている
- エラーメッセージを読むことでデバッグが効率的になる
- 関数名はケースセンシティブで完全一致する必要がある
- テンプレートリテラルは文字列と変数を組み合わせる際に読みやすいコードになる
...
### メモ 📌
- 変数の命名規則についても復習する必要がある
- 日本語変数名も使えるが、英語の変数名の方が一般的
...
### メモ 📌
- 学習時間：22:00-22:30（0時間30分）
- 聞き流し学習は通勤時間などの隙間時間を活用すると効率的
